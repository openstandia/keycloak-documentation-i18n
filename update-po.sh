...
if [ "$?" -eq 0 ]; then
    ...
    docker run --rm -it -v $(pwd):/build -w /build -u $UID:$GID openstandia/keycloak-documentation-i18n:po4a-1e6c31274c4c49a6dbefa33293de692b67970d68 po4a --no-translations --package-name="keycloak-documentation-i18n" --package-version=" " --copyright-holder="Nomura Research Institute, Ltd." --msgmerge-opt '--previous' po4a.cfg
fi
